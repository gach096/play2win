...
*/
    function validarDatos($nomApe, $tarj, $mes, $año){
        ...
        if (ctype_digit($nomApe)) throw new Exception("El nombre no es válido.");
        if (!ctype_digit($tarj)) throw new Exception("La tarjeta debe ser numérica.");
        if (!ctype_digit($mes)) throw new Exception("El mes debe ser numérico.");
        if (!ctype_digit($año)) throw new Exception("El año debe ser numérico.");
        if (strlen($tarj) != 16) throw new Exception("La tarjeta debe tener 16 dígitos.");
        if (strlen($mes) < 1 || strlen($mes) > 2) throw new Exception("Mes incorrecto.");
        if (strlen($año) != 4) throw new Exception("El año debe tener 4 dígitos.");
        if ($mes < 1 || $mes > 12) throw new Exception("El mes debe estar entre 1 y 12.");
        if ($año < $hoy['year'] || ($año == $hoy['year'] && $mes < $hoy['mon'])) throw new ValidacionException("La tarjeta está vencida.");
		if ($año > ($hoy['year'] + 5)) throw new ValidacionException("Año no válido.");
